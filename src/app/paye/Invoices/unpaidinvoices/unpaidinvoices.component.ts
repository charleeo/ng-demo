...
export class UnpaidinvoicesComponent implements OnInit {
  ...
  constructor(private httpClient: HttpClient,
    ...
    this.getUnpaidInvoices(this.config.itemsPerPage, this.config.currentPage);
  ...
  getUnpaidInvoices(perpage, pageno) {
    ...
    this.httpClient.post<any>(this.apiUrl, obj, { headers: reqHeader} ).subscribe(data => {
      //this.apidata = data.response.data.filter(x => x.payment_status == 0);
      this.apidata = data.response.data;
      this.config.totalItems = data.response.total;
      console.log("unPaidInvoicesData", this.apidata);
  ...
  pageChange(newPage: number) {
    this.apidata = "";
    this.router.navigate(["/unpaidinvoices"], { queryParams: { page: newPage } });
    this.getUnpaidInvoices(this.config.itemsPerPage, newPage);
  }
  ...
  public setItemsPerPage(event) {
    ...
    this.apidata = "";
    this.router.navigate(["/unpaidinvoices"]);
    this.getUnpaidInvoices(this.config.itemsPerPage, 1);
  }
}
