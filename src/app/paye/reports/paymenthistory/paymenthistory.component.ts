...
export class PaymenthistoryComponent implements OnInit {
  ...
  paymentHistoriesData: any;
  searchForm: FormGroup;
  apiResponseMessage: any;
  taxOffice: any;
  title = 'PAYE - Payment History Report'
  showBackToDashboard: boolean;
  redirectFromPaymentUrl: string;
  ...
  ngOnInit(searchObj = null) {
    ...
    if (searchObj == null || searchObj == '') {
      ...
      this.getPaymentHistories(this.config.itemsPerPage, this.config.currentPage);
      ...
      this.paymentHistoriesData = '';
      // this.getPaymentHistories(searchObj);
    ...
    let backToDashboard = localStorage.getItem("redirect_payment_to_dashboard");

    if (backToDashboard == "1") {
      this.redirectFromPaymentUrl = '/dashboard';
    } else {
      this.redirectFromPaymentUrl = '/viewreports';
    }
  ...
  getPaymentHistories(perpage, pageno) {
    this.spinnerService.show();
    this.apiUrl = `${environment.AUTHAPIURL}transactions/list?page=${pageno}`;
    ...
    console.log('werty ', this.searchObject )

    this.httpClient.post<any>(this.apiUrl, this.searchObject, { headers: reqHeader }).subscribe(data => {
      console.log("paymentHistoriesData: ", data);
      this.paymentHistoriesData = data.response == null ? [] : data.response.data;
      this.config.totalItems = data.response.total;
      this.apiResponseMessage = data.message;

      if (this.apiResponseMessage.includes('No') && this.searchObject != null) {
        Swal.fire({
          icon: 'info',
          title: 'Information',
          text: this.apiResponseMessage,
  ...
  viewCorporate(modal, selectedCorporate) {
    ...
    this.companyName = selectedCorporate.company_name;
  ...
  loadSelectedCorporateData(selectedCorporate) {
    this.corporateForm = this.formBuilder.group({
      ...
      companyName: [selectedCorporate.company_name, [Validators.required, Validators.pattern('^[A-Za-z 0-9 _-]*[A-Za-z0-9][A-Za-z0-9 _]*$'), Validators.maxLength(60)]],
  ...
  getPaymentHistory() {
    ...
    this.httpClient
      ...
      .subscribe((data) => {
        console.log(data);
        this.spinnerService.hide();
        this.apidata = data.response.data.reverse();
  ...
  viewPayment(modal, selectedPayment) {
    ...
    this.showModal(modal);
    this.getSinglePayment(selectedPayment.id);
  }

  getSinglePayment(paymentId) {
    this.spinnerService.show();
    this.apiUrl = `${environment.AUTHAPIURL}payments/${paymentId}?corporate_id=${this.selectedCorporateId}`;
  ...
  onSubmitSearch(formAllData: any) {
    ...
    this.searchObject = {
      ...
      channel: formAllData.paymentChannelId,
      company_name: formAllData.companyName,
      revenue_stream: formAllData.revenueStreamId,
      date_start: this.daterange.start !== undefined ? this.daterange.start : null,
      date_end: this.daterange.end !== undefined ? this.daterange.end : null,
      business_id: formAllData.business_id,
      business_name: formAllData.business_name,
  ...
  taxOfficeChanged(taxOfficeId) {
    this.taxOffice = this.taxTaxOffices.filter(x => x.id == taxOfficeId)[0]?.name;
  }

  pageChange(newPage: number) {
    this.paymentHistoriesData = "";
    this.router.navigate(["/paymenthistory"], { queryParams: { page: newPage } });
    this.getPaymentHistories(this.config.itemsPerPage, newPage);
  }
  ...
  public setItemsPerPage(event) {
    ...
    this.getPaymentHistories(this.config.itemsPerPage, 1);
  }
