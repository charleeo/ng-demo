...
import { ModalDismissReasons, NgbModal, NgbModalOptions, NgbDate, NgbCalendar } from '@ng-bootstrap/ng-bootstrap';
...
@Component({
  selector: 'app-projectionreports',
  templateUrl: './projectionreports.component.html',
  styleUrls: ['./projectionreports.component.css']
})
export class ProjectionreportsComponent implements OnInit {
  dtOptions: any = {};
  modalOptions: NgbModalOptions;
  apiUrl: string;
  projectionsApiData: any;
  searchForm: FormGroup;
  submitted: boolean;
  apiResponseMessage: any;
  title = 'PAYE - Projection Report'
  taxTaxOffices: any;
  roleID: string;
  ...
  ngOnInit(searchObj = null): void {
    ...
    if (searchObj == null || searchObj == '') {
      ...
      this.getProjections(this.config.itemsPerPage, this.config.currentPage);
  ...
  getProjections(perpage, pageno) {
    this.apiUrl = `${environment.AUTHAPIURL}projections-by-corporates?page=${pageno}`;
    ...
    this.httpClient.post<any>(this.apiUrl, this.searchObject, { headers: reqHeader }).subscribe((data) => {
      console.log("projectionsApiData: ", data);
      this.projectionsApiData = data.response == null ? [] : data.response.data;
  ...
  onSubmitSearch(formAllData: any) {
    ...
    this.searchObject = {
      projection_year: formAllData.taxYear,
      company_name: formAllData.companyName,
      app_id: formAllData.filedFromId == "1" ? 1 : formAllData.filedFromId == "2" ? 2 : null,
      date_from: this.daterange.start !== undefined ? this.daterange.start : null,
      date_to: this.daterange.end !== undefined ? this.daterange.end : null,
  ...
  pageChange(newPage: number) {
    this.projectionsApiData = "";
    this.router.navigate(["/projectionreports"], { queryParams: { page: newPage } });
    this.getProjections(this.config.itemsPerPage, newPage);
  }
  ...
  public setItemsPerPage(event) {
    ...
    this.projectionsApiData = "";
    this.router.navigate(["/projectionreports"]);
    this.getProjections(this.config.itemsPerPage, 1);
  }
}
