...
export class LandsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.getLands(this.config.itemsPerPage, this.config.currentPage);
  ...
  getLands(perpage, pageno) {
    this.landsData = "";
    this.apiUrl = `${environment.AUTHAPIURL}lands/index?page=${pageno}`;
    ...
    this.httpClient.post<any>(this.apiUrl, this.searchObject, { headers: reqHeader }).subscribe((data) => {
        console.log("landsData", data);
        if (data.status == true) {
          this.landsData = data.response.data;
  ...
  pageChange(newPage: number) {
    this.landsData = "";
    this.router.navigate(["/lands"], { queryParams: { page: newPage } });
    this.getLands(this.config.itemsPerPage, newPage);
  }
  ...
  public setItemsPerPage(event) {
    ...
    this.getLands(this.config.itemsPerPage, 1);
  }

  addLand(modal) {
    this.addLandModalRef = this.utilityService.openModal(modal);
  }

  viewLand(modal, selectedLand) {
    this.showLinkTaxpayerButton = false;
    this.selectedLandId = selectedLand.id;
    this.getLinkedTaxpayerData();
    this.viewLandModalRef = this.utilityService.openModal(modal);
  }
  ...
  onSubmitSearch(formAllData: any) {
    ...
    this.searchObject = {
      land_name: formAllData.landName,
  ...
  getLinkedTaxpayerData() {
    this.apiUrl = `${environment.AUTHAPIURL}land/taxpayers`;
  ...
  unlinkTaxpayer(selectedaxpayer) {
    ...
    if (selectedaxpayer.business_id !== null) {
      obj = {
        business_id: this.selectedLandId,
