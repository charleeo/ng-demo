...
export class BuildingsComponent implements OnInit {
  ...
  buildingsData: any = [];
  apiUrl: string;
  addBuildingModalRef: any;
  submitted: boolean = false;
  selectedBuildingId: any;
  viewBuildingModalRef: any;
  linkedTaxpayerData: any;
  viewTaxpayerModalRef: any;
  corporateTaxpayer: boolean;
  individualTaxpayer: boolean;
  selectedCorporateId: any;
  selectedIndividualId: any;
  showLinkTaxpayerButton: any;
  linkTaxpayerModalRef: any;
  assessmentData: any = [];
  assetTypeId: any = 3; // Building
  managerRole: boolean;
  ...
  ngOnInit(): void {
    ...
    this.getBuildings(this.config.itemsPerPage, this.config.currentPage);
  ...
  getBuildings(perpage, pageno) {
    this.buildingsData = "";
    this.apiUrl = `${environment.AUTHAPIURL}buildings?page=${pageno}`;
    ...
    // this.searchObject["status"] = "unlinked";
    ...
    this.httpClient.post<any>(this.apiUrl, this.searchObject, { headers: reqHeader }).subscribe((data) => {
        ...
        if (data.status == true) {
          this.buildingsData = data.response.data;
  ...
  pageChange(newPage: number) {
    this.buildingsData = "";
    this.router.navigate(["/buildings"], { queryParams: { page: newPage } });
    this.getBuildings(this.config.itemsPerPage, newPage);
  }
  ...
  public setItemsPerPage(event) {
    ...
    this.getBuildings(this.config.itemsPerPage, 1);
  }

  addBuilding(modal) {
    this.addBuildingModalRef = this.utilityService.openModal(modal);
  }

  viewBuilding(modal, selectedBuilding) {
    this.showLinkTaxpayerButton = false;
    this.selectedBuildingId = selectedBuilding.id;
    this.getLinkedTaxpayerData();
    this.viewBuildingModalRef = this.utilityService.openModal(modal);
  }
  ...
  onSubmitSearch(formAllData: any) {
    ...
    this.searchObject = {
      building_name: formAllData.buildingName,
    };
  ...
  getLinkedTaxpayerData() {
    this.apiUrl = `${environment.AUTHAPIURL}building/taxpayers`;
    ...
    this.httpClient.post<any>(this.apiUrl, jsonData, { headers: reqHeader }).subscribe((data) => {
        // console.log("linkedTaxpayerData: ", data);
        if (data.status == true) {
          this.linkedTaxpayerData = data.response.data;
          this.spinnerService.hide();
        }
        else {
          this.spinnerService.hide();
          this.linkedTaxpayerData = [];
  ...
  unlinkTaxpayer(selectedaxpayer) {
    var obj = {};

    if (selectedaxpayer.business_id !== null) {
      obj = {
        building_id: this.selectedBuildingId,
        corporate_id: selectedaxpayer.corporate_id,
        individual_id: selectedaxpayer.individual_id,
      }
    }
    ...
    Swal.fire({
      ...
      if (result.value) {
        this.postUnlinkTaxpayer(obj);
      }
    });

  }

  postUnlinkTaxpayer(objData) {
    ...
    this.httpClient.post<any>(this.apiUrl, objData, { headers: reqHeader }).subscribe((data) => {
        if (data.status == true) {
          Swal.fire({
            icon: "success",
            title: "Success",
            text: "Taxpayer has been unlinked successfully",
            showConfirmButton: true,
            timer: 5000,
          });
  ...
  viewTaxpayer(selectedTaxpayer, modal) {
    console.log("selectedTaxpayer: ", selectedTaxpayer);
    this.viewTaxpayerModalRef = this.utilityService.openModal(modal);

    if (selectedTaxpayer.corporate_id !== null) {
      this.corporateTaxpayer = true;
      this.individualTaxpayer = false;
      this.selectedCorporateId = selectedTaxpayer.corporate_id;
    }

    if (selectedTaxpayer.individual_id !== null) {
      this.corporateTaxpayer = false;
      this.individualTaxpayer = true;
      this.selectedIndividualId = selectedTaxpayer.individual_id;
    }

    // this.showModal(modal);
  }

  toggleLinkTaxpayerButton(showLinkTaxpayerButton) {
    if (this.roleID == "2" || this.roleID == "3") {
      this.showLinkTaxpayerButton = showLinkTaxpayerButton;
    }
  }

  showLinkTaxpayerModal(modal) {
    this.linkTaxpayerModalRef = this.utilityService.openModal(modal);
  }
